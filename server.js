...
function broadcast(room, builder) { for (const p of room.players) safeSend(p.ws, builder(p.id)); }
function deck() {
  const d = [];
...
function evaluate7(cards){
  ...
  if(straightFlush) return {cat:8,t:[straightFlush],name:'同花顺'};
  ...
  if(flushSuit) return {cat:5,t:sortDesc(flushCards).slice(0,5),name:'同花'};
  ...
  if(byCount[0][1]===2 && byCount[1] && byCount[1][1]===2){
    const hi=Math.max(byCount[0][0],byCount[1][0]), lo=Math.min(byCount[0][0],byCount[1][0]);
    const k=sortDesc(vals.filter(v=>v!==hi&&v!==lo))[0];
  ...
  return {cat:0,t:sortDesc(vals).slice(0,5),name:'高牌'};
}
function cmpEval(a,b){
  if(a.cat!==b.cat) return a.cat-b.cat;
  for(let i=0;i<Math.max(a.t.length,b.t.length);i++){ const av=a.t[i]||0, bv=b.t[i]||0; if(av!==bv) return av-bv; }
  return 0;
}
function nextOccupied(room, start){
  const n=room.players.length;
  for(let i=1;i<=n;i++){ const idx=(start+i)%n; if(room.players[idx].chips>0) return idx; }
  return 0;
}
...
function collectToPot(room){
  room.players.forEach(p=>{ room.pot += p.bet; p.bet = 0; });
}
...
function startHand(room){
  ...
  room.phase='playing'; room.street='preflop'; room.pot=0; room.community=[]; room.deck=deck();
  room.dealerIndex = nextOccupied(room, room.dealerIndex);
  room.players.forEach(p=>{
    p.hand=[]; p.folded=false; p.inHand=p.ready && p.chips>0; p.bet=0; p.acted=false;
  ...
  const sbAmt=Math.min(SMALL_BLIND, room.players[sb].chips);
  const bbAmt=Math.min(BIG_BLIND, room.players[bb].chips);
  room.players[sb].chips -= sbAmt; room.players[sb].bet = sbAmt;
  room.players[bb].chips -= bbAmt; room.players[bb].bet = bbAmt;
  room.currentBet = bbAmt;
  const first = nextActive(room, bb);
  setTurn(room, first);
  room.message = '翻前行动';
  return true;
}
...
function awardSingle(room, winner, msg){
  ...
  room.phase='waiting'; room.street='showdown'; room.turnPlayerId=null; room.message=msg;
}
function showdown(room){
  ...
  for(const p of contenders){
    ...
    if(!best || cmpEval(p.eval, best.eval)>0){ best=p; winners=[p]; }
  ...
  room.message = winners.length===1 ? `${winners[0].name} 获胜：${winners[0].eval.name}` : `平分底池：${winners.map(w=>w.name).join(' / ')}`;
}
function advanceStreet(room){
  collectToPot(room);
  const alive = remaining(room);
  if(alive.length===1){ awardSingle(room, alive[0], `${alive[0].name} 直接收池`); return; }
...
function maybeAdvance(room){
  const alive = remaining(room);
  if(alive.length===1){ awardSingle(room, alive[0], `${alive[0].name} 对手全弃牌`); return; }
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', raw => {
    ...
    if(msg.type==='create_room'){
      ...
      const room={roomId,players:[],dealerIndex:0,deck:[],community:[],pot:0,phase:'waiting',street:'preflop',currentBet:0,turnPlayerId:null,message:'房间创建成功'};
      room.players.push({id:playerId,name:msg.name||'玩家1',ws,seat:1,chips:STARTING_CHIPS,ready:false,hand:[],folded:false,inHand:false,bet:0,acted:false,isDealer:false,isSB:false,isBB:false});
    ...
    if(msg.type==='join_room'){
      ...
      room.players.push({id:playerId,name:msg.name||('玩家'+(room.players.length+1)),ws,seat:room.players.length+1,chips:STARTING_CHIPS,ready:false,hand:[],folded:false,inHand:false,bet:0,acted:false,isDealer:false,isSB:false,isBB:false});
