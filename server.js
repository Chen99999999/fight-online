...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'join_room') {
      ...
      if (!room) {
        safeSend(ws, { type: 'error_msg', text: '房间不存在' });
        return;
      }
      if (room.players.size >= 2) {
        safeSend(ws, { type: 'error_msg', text: '房间已满' });
        return;
      }
    ...
    if (msg.type === 'reset_match') {
      for (const p of room.players.values()) {
        p.state.hp = 100;
        p.state.x = p.role === 'p1' ? 180 : 620;
        p.state.y = 310;
        p.state.facing = p.role === 'p1' ? 1 : -1;
        p.state.action = 'idle';
      }
