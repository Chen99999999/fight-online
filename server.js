...
function eval5(cards) {
  ...
  if (byCount[0][1] === 4) {
    ...
    const k = sortDesc(vals.filter(v => v !== four))[0];
    return { cat: 7, t: [four, k], name: '四条' };
  }
  ...
  if (byCount[0][1] === 3) {
    const tr = byCount[0][0];
    const ks = sortDesc(vals.filter(v => v !== tr)).slice(0, 2);
    return { cat: 3, t: [tr, ...ks], name: '三条' };
  }
  ...
  if (byCount[0][1] === 2) {
    const pr = byCount[0][0];
    const ks = sortDesc(vals.filter(v => v !== pr)).slice(0, 3);
    return { cat: 1, t: [pr, ...ks], name: '一对' };
  }
...
function publicPlayer(p, reveal = false) {
  return {
    ...
    showdownName: p.showdownName || '',
    visibleCards: reveal ? (p.hand || []) : [],
    highlightCards: reveal ? (p.highlightCards || []) : []
  };
}
...
function resetStreet(room) {
  room.players.forEach(p => { p.bet = 0; p.acted = false; p.actionText = ''; });
  room.currentBet = 0;
}
...
function showdown(room) {
  ...
  for (const p of contenders) {
    ...
    p.highlightCards = out.cards;
    if (!best || cmpEval(out.eval, best.eval) > 0) {
      best = { player: p, eval: out.eval };
      winners = [p];
  ...
  room.players.forEach(p => {
    if (winners.some(w => w.id === p.id)) p.selfResult = `${p.showdownName}，你赢了`;
    else p.selfResult = `${p.showdownName || '高牌'}，你输了`;
  });
}
...
wss.on('connection', ws => {
  ...
  ws.on('message', raw => {
    ...
    if (msg.type === 'fold') {
      player.folded = true;
      player.acted = true;
      player.actionText = '弃牌';
      pushAction(room, `${player.name} 弃牌`);
      maybeAdvance(room);
      broadcast(room, id => ({ type: 'room_state', ...roomStateFor(room, id) }));
      return;
    }
